# Event: [Name] Engagement

## Audience Outreach

> _How will this event be promoted? Identify specific channels that will be used._

_[Enter specific channel information here]_

**Additional outreach notes:**

_[Enter any additional details here]_

## Audience Engagement

> _How will the audience be engaged before, during, and after the event? Select all that apply._

**Selected engagement methods:**

- [ ] Polling
- [ ] Quizzes
- [ ] Pre Q&A
- [ ] Live Q&A
- [ ] Live chat
- [ ] Video sharing
- [ ] Participant list visible
- [ ] Shared attendee list (with contact info, social media links)
- [ ] Shoutouts to acknowledge participants in the room and behind screen
- [ ] Breakout room discussions ("virtual cocktail tables")
- [ ] Ice breakers
- [ ] Ask guests to prepare: wear blue and gold, bring an object to share/display, bring a "virtual offering", bring a beverage (coffee chat, happy hour)
- [ ] Event is recorded and offered as video on demand (VOD)
- [ ] Send things to people in advance (branded masks, headphones, campaign or bear pins)

**Additional engagement notes:**

_[Enter any additional details here]_

## Registration and Attendance

> _What platform will be used to track registration and attendance, and what fields are needed? Update with links to registration applications and decisions, and attendance sheets._

_[Enter application information and relevant links here.]_

## Follow Ups

**Post-event**
> _What is the follow-up? Is there a survey? Call to action?_

_[Enter your response here]_

## Promotion

> _How will the event be promoted (social media, email lists, etc.)? Include relevant links to social media posts once they have been created._

_[Enter promotion plan here]_

## Design Deliverables

> _Include links to final design assets._

_[Enter links here]_

## Risks

> _What engagement/communications risks exist (e.g., low open rates, missed deadlines, approval delays)? How will they be mitigated?_

_[Enter identified engagement risks here]_

